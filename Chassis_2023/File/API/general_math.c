...
//
//	函数名称：my_sqrt_reciprocal(float number)
//	函数功能：平方根倒数速算法，进行两次牛顿迭代法迭代
//
...
{	
	...
	y = * ( float * ) &i;
	y = y * ( 1.5f - ( x * y * y ) );	//第一次牛顿迭代法迭代
	y = y * ( 1.5f - ( x * y * y ) );	//第二次牛顿迭代法迭代
...
//
//	函数名称：mx_sin(double rad)
//	函数功能：计算正弦值，计算范围-pi/2~pi/2
//
...
//
//	函数名称：my_sin(double rad)
//	函数功能：若输入角度大于pi，则减pi后使用mx_sin()计算正弦值
//
