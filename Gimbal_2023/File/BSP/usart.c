...
/*-------------------USART3_TX的DMA初始化------------------*/	
	...
	while (DMA_GetCmdStatus(USART3_TX_STREAM) != DISABLE){};
		...
		DMA_InitStructure.DMA_Memory0BaseAddr=(u32)(&ChassisCushioning_Tx);
...
/*----------------------引脚复用---------------------------*/	
	GPIO_PinAFConfig(GPIOA,GPIO_PinSource0,GPIO_AF_UART4); 								// GPIOC10复用为UART4 TX
	GPIO_PinAFConfig(GPIOA,GPIO_PinSource1,GPIO_AF_UART4); 								// GPIOC11复用为UART4 RX
